...
def test_load_csv_file_not_found():
    with patch("os.path.join", return_value="nonexistent.csv"), patch(
        "pandas.read_csv"
    ) as mock_read_csv:
...
def test_load_csv_parser_error():
    with patch("os.path.join", return_value="bad.csv"), patch(
        "pandas.read_csv"
    ) as mock_read_csv:
...
def test_process_data_columns_exist_error(mock_df, mock_engine):
    with patch("src.data.load_csv", return_value=mock_df), patch(
        "src.data.columns_exist", return_value=False
    ):
...
def test_process_data_write_data_error(mock_df, mock_engine):
    with patch("src.data.load_csv", return_value=mock_df), patch(
        "src.data.columns_exist", return_value=True
